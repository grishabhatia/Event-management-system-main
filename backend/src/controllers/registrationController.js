...
export const registerForEvent = async (req, res) => {
  try {
    ...
    // Check active registrations count for capacity
...
export const promoteFromWaitlist = async (eventId) => {
  ...
  try {
    await sendEmail({
      ...
      html: `<p>You have been promoted from the waitlist.</p><p>Your registration for ${nextRegistration.event.title} is now confirmed.</p>`,
    });
  } catch (_) { }
